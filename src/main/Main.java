...
//		String tipeBarangV; //Vessel tipeBarang;
		...
		TransactionHeader TH = new TransactionHeader(transactionID, transactionDate, cashierID, mekanikID, status, keluhan);
		TransactionDetail TD = new TransactionDetail(customerID, transactionID, quantity, barangID, tipeBarang, hargaBarang);
		
		String queryInsertTransactionHeader = String.format("INSERT INTO TransactionHeader VALUES('%s', '%s', '%s', '%s', '%s', '%s')", 
				TH.getTransactionID(), TH.getTransactionDate(), TH.getCashierID(), TH.getMekanikID(), TH.getStatus(), TH.getKeluhan());
...
//				customerKeluhanlist.add(rs.getString("CustomerComplaints"));
	...
	private void viewTransactionHeader() {
		...
		System.out.println("=====================================================================================================");
		System.out.println("| ID Transaction | Tgl Transaction|   ID Cashier   |   ID Mekanik  |    Status     |     Keluhan    |");
		System.out.println("=====================================================================================================");
		
		for (int i = 0; i < transactionIDList.size(); i++) {
	        System.out.printf("| %-14s | %-14s | %-14s | %-14s | %-14s | %-14s |\n",
	        		transactionIDList.get(i), transactionDateList.get(i),cashierIDList.get(i),mekanikIDList.get(i),statusList.get(i), keluhanList.get(i));
	    }
		System.out.println("=====================================================================================================");
	...
	private void updateStatusMekanik() {
		...
		do {
			...
			if (statusDecision == 1) {
				...
				String query = String.format("UPDATE TransactionHeader SET Status = 'Selesai' WHERE TransactionID = '%s'", transactionIDsearch);
	...
	private void viewUC_Transaction() {
		...
		System.out.println("====================================================================");
		System.out.println("| ID Transaction | Tgl Transaction|    Status     |     Keluhan    |");
		System.out.println("====================================================================");
		
		for (int i = 0; i < UCtransactionIDList.size(); i++) {
	        System.out.printf("| %-14s | %-14s | %-14s | %-14s |\n",
	        		UCtransactionIDList.get(i), UCtransactionDateList.get(i),UCstatusList.get(i), UCkeluhanList.get(i));
	    }
		System.out.println("=====================================================================");
	}
	...
	private void confirmOrder() {
		...
		do {
			...
			insertMigrationUCTQuery = String.format("INSERT INTO TransactionHeader(TransactionID, TransactionDate, CashierID, MekanikID, Status, Keluhan)"
					+ " VALUES('%s','%s','%s','%s',"
					+ "(SELECT StatusPemesanan FROM UC_Transaction WHERE TransactionID = '%s'),"
					+ "(SELECT Keluhan FROM UC_Transaction WHERE TransactionID = '%s'))", 
					transactionID, transactionDate, cashierID, mekanikID, transactionID, transactionID);
